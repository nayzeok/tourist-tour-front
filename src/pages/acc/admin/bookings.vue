cat > ~/Documents/Tourist-Tour/travel-front/src/pages/acc/admin/bookings.vue << 'EOF'
<script setup lang="ts">
import { computed, ref } from 'vue'
import { fetchAllBookings } from '~/src/shared/api/user'
import { useAuthStore } from '~/src/shared/store'
import { formatISOToDate, formatRubles } from '~/src/shared/utils'
import type { AdminBooking } from '~/src/shared/types'
...
const getUserDisplayName = (user: AdminBooking['user']) => {
  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="mt-1 text-sm text-gray-500">
        Все бронирования на сайте (всего: {{ total }})
      </p>
    </div>
      ...
      <div class="grid gap-4">
        ...
        >
          ...
          <div class="mt-3 rounded-lg bg-gray-50 p-3">
            <p class="text-xs uppercase text-gray-500 mb-1">Клиент</p>
            <p class="font-medium">{{ getUserDisplayName(booking.user) }}</p>
            <p class="text-sm text-gray-600">{{ booking.user.email }}</p>
            <p v-if="booking.user.phone" class="text-sm text-gray-600">{{ booking.user.phone }}</p>
          </div>
